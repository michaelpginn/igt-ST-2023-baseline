...
def main(mode: str, lang: str, track: str, pretrained_path: str, encoder_path: str, data_path: str):
    ...
    if mode == 'train':
        ...
        preds = np.argmax(preds, axis=2)
        input_lengths = [len(tokenizer(transcription)) for transcription in predict_data['transcription']]
        write_predictions(data_path, lang=lang, preds=preds, pred_lengths=input_lengths, encoder=encoder, from_vocabulary_index=2)
